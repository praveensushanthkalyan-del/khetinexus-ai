...
Respond STRICTLY in valid JSON matching this schema:
{
  ...
  app.get('/api/providers/weather', async (req, res) => {
    try {
      ...
      if (
        cachedWeather &&
        Date.now() - cachedWeather.fetchedAt < WEATHER_CACHE_TTL_MS
      ) {
        omData = cachedWeather.data;
      } else {
        // Reuse an already-running request for the same coordinates.
        let weatherRequest = weatherApiInflight.get(cacheKey);
        ...
        if (!weatherRequest) {
          ...
          weatherApiInflight.set(cacheKey, weatherRequest);
        }
